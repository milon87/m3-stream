...
public class FlvWriter {
    ...
    public FlvWriter(int seekTime, String fileName) {
        ...
        if(fileName == null) {
        	Log.i(this.getClass().getName(),"save file notspecified, will only consume stream");
            out = null;
            return;
        }
        try {
            ...
            Log.i(this.getClass().getName(),"opened file for writing: "+ file.getAbsolutePath());
        } catch (Exception e) {
    ...
    public void close() {
        ...
        Log.i(this.getClass().getName(),"final media duration: "+((channelTimes[primaryChannel] - seekTime) / 1000)+" seconds (seek time: "+(seekTime / 1000)+")");
    }

    private void logWriteProgress() {
        final int seconds = (channelTimes[primaryChannel] - seekTime) / 1000;
        if (seconds >= lastLoggedSeconds + 10) {
        	Log.i(this.getClass().getName(),"write progress: " + seconds + " seconds");
    ...
    public void write(RtmpMessage message) {
        ...
        if(header.isAggregate()) {
            ...
            if(primaryChannel == -1 && (header.isAudio() || header.isVideo())) {
            	Log.i(this.getClass().getName(),"first media packet for channel: "+ header);
