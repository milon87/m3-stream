...
public class MVHD implements Payload {
    ...
    public void read(ChannelBuffer in) {
        version = in.readByte();
        Log.d(this.getClass().getName(),"version: "+ Utils.toHex(version));
        flags = new byte[3];
        in.readBytes(flags);
        if (version == 0x00) {
        ...
        volume = in.readShort();
        Log.d(this.getClass().getName(),"creationTime "+creationTime+" modificationTime "+modificationTime+" timeScale "+timeScale+" duration "+duration+" playbackRate "+playbackRate+" volume "+volume);
        ...
        for (int i = 0; i < transformMatrix.length; i++) {
            ...
            Log.d(this.getClass().getName(),"transform matrix["+i+"]: "+ transformMatrix[i]);
        }
        reserved3 = new int[6];
